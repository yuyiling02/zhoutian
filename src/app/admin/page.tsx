...
import { reorderCreativeCategory } from '@/lib/creative-work-order';
import { ArrowLeft, Check, Edit3, ImageIcon, ImagePlus, Layers3, LoaderCircle, Lock, LogOut, Plus, Search, Shapes, Trash2, Upload, X } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const moveToPosition = (index: number, targetIndex: number) => {
    if (query) return;
    const length = zone === '3d' ? artworks.length : activeCreative.length;
    if (index < 0 || index >= length || targetIndex < 0 || targetIndex >= length || index === targetIndex) return;
    if (zone === '3d') {
      setArtworks((current) => {
        const next = [...current];
      ...
      setCreative((current) => reorderCreativeCategory(current, zone, index, targetIndex));
    }
    setOrderDirty(true);
  };
    ...
    <div className="museum-grid min-h-screen text-[#172033]" style={{ '--zone': definition.color, '--zone-soft': definition.softColor } as CSSProperties}>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <section className="admin-surface mt-5 rounded-[2rem] p-4 sm:p-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-2xl font-black">{definition.title}</h2><p className="mt-1 text-sm text-slate-500">直接输入展示序号，可编辑资料或删除作品</p></div><div className="flex gap-2"><div className="relative flex-1 sm:w-72"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索作品或作者" className="h-11 rounded-full border-slate-900/10 pl-10" />{query && <button type="button" onClick={() => setQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2"><X className="size-4" /></button>}</div>{orderDirty && <Button onClick={() => void saveOrder()} className="rounded-full bg-slate-950 font-black text-white"><Check className="mr-2 size-4" />保存排序</Button>}</div></div>
          ...
          {loading ? <div className="grid min-h-72 place-items-center text-slate-500"><LoaderCircle className="size-8 animate-spin" /></div> : zone === '3d' ? <ArtworkList items={visibleArtworks} allItems={artworks} query={query} move={moveToPosition} edit={(work) => openEdit({ kind: '3d', value: work })} remove={remove3D} /> : <CreativeList items={visibleCreative} allItems={activeCreative} query={query} zone={zone} move={moveToPosition} edit={(work) => openEdit({ kind: 'creative', value: work })} remove={removeCreative} />}
        </section>
      </main>
...
function ArtworkList({ items, allItems, query, move, edit, remove }: { items: Artwork[]; allItems: Artwork[]; query: string; move: (index: number, targetIndex: number) => void; edit: (work: Artwork) => void; remove: (work: Artwork) => void }) { if (!items.length) return <Empty />; return <div className="mt-6 space-y-3">{items.map((work) => { const index = allItems.findIndex((item) => item.id === work.id); return <article key={work.id} className="grid gap-4 rounded-2xl border border-slate-900/10 bg-white p-4 transition hover:border-[#3C5CFF]/30 sm:grid-cols-[72px_1fr_auto] sm:items-center"><div className="relative size-18 overflow-hidden rounded-xl bg-[#E9EDFF]">{work.thumbnail ? <Image src={work.thumbnail} alt="" fill sizes="72px" className="object-cover" /> : <span className="grid size-full place-items-center text-[#3C5CFF]"><ZoneIcon name="paint-space" className="size-6" /></span>}</div><div className="min-w-0"><h3 className="truncate font-black">{work.title}</h3><p className="mt-1 truncate text-sm text-slate-500">{work.author} · ID {work.id}</p></div><RowActions index={index} length={allItems.length} query={query} move={move} edit={() => edit(work)} remove={() => remove(work)} /></article>; })}</div>; }
function CreativeList({ items, allItems, query, zone, move, edit, remove }: { items: CreativeWork[]; allItems: CreativeWork[]; query: string; zone: CreativeCategory; move: (index: number, targetIndex: number) => void; edit: (work: CreativeWork) => void; remove: (work: CreativeWork) => void }) { const def = getZoneDefinition(zone); if (!items.length) return <Empty />; return <div className="mt-6 grid gap-4 md:grid-cols-2">{items.map((work) => { const index = allItems.findIndex((item) => item.id === work.id); const title = work.title.trim(); const author = work.author.trim(); const primaryLabel = title || author; return <article key={work.id} className="overflow-hidden rounded-2xl border border-slate-900/10 bg-white transition" style={{ boxShadow: `0 6px 0 ${def.color}10` }}><div className="relative aspect-[16/9]" style={{ backgroundColor: def.softColor }}><Image src={work.imageUrl} alt={title || (author ? `${author}的作品` : '作品图片')} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" /></div><div className="p-4">{primaryLabel && <h3 className="truncate font-black">{primaryLabel}</h3>}{title && author && <p className="mt-1 truncate text-sm text-slate-500">{author}</p>}<p className={`${primaryLabel ? 'mt-1' : ''} truncate text-xs text-slate-400`}>ID {work.id}</p><div className="mt-4"><RowActions index={index} length={allItems.length} query={query} move={move} edit={() => edit(work)} remove={() => remove(work)} /></div></div></article>; })}</div>; }
function RowActions({ index, length, query, move, edit, remove }: { index: number; length: number; query: string; move: (index: number, targetIndex: number) => void; edit: () => void; remove: () => void }) { return <div className="flex flex-wrap items-center gap-2"><OrderInput index={index} length={length} disabled={Boolean(query)} move={move} /><Button variant="outline" onClick={edit} className="rounded-full"><Edit3 className="mr-2 size-4" />编辑</Button><Button variant="outline" size="icon" onClick={remove} className="rounded-full text-red-600 hover:text-red-700" aria-label="删除"><Trash2 className="size-4" /></Button></div>; }
function OrderInput({ index, length, disabled, move }: { index: number; length: number; disabled: boolean; move: (index: number, targetIndex: number) => void }) { const [value, setValue] = useState(String(index + 1)); useEffect(() => { setValue(String(index + 1)); }, [index]); const commit = () => { const position = Number(value); if (!Number.isInteger(position) || position < 1 || position > length) { setValue(String(index + 1)); return; } move(index, position - 1); }; return <label className="flex h-10 items-center gap-2 rounded-full border border-slate-900/10 bg-slate-50 px-3 text-xs font-black text-slate-500"><span>序号</span><Input type="number" inputMode="numeric" min={1} max={length} value={value} disabled={disabled} onFocus={(event) => event.currentTarget.select()} onChange={(event) => setValue(event.target.value)} onBlur={commit} onKeyDown={(event) => { if (event.key === 'Enter') event.currentTarget.blur(); }} aria-label={`展示序号，范围 1 到 ${length}`} className="h-7 w-14 rounded-lg border-slate-900/10 bg-white px-2 text-center font-black text-slate-950" /></label>; }
