...
import { ArrowDown, ArrowLeft, ArrowUp, Box, Check, Edit3, ImagePlus, LoaderCircle, Lock, LogOut, Plus, Search, Trash2, Upload, X } from 'lucide-react';
...
export default function AdminPage() {
  ...
  return (
    <div className="museum-grid min-h-screen bg-[#F7F7F4] text-slate-950" style={{ '--zone': definition.color } as CSSProperties}>
      <header className="museum-glass sticky top-0 z-30 border-b border-slate-900/8"><div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8"><Link href="/" className="inline-flex items-center gap-2 text-sm font-black"><ArrowLeft className="size-4" /><span className="hidden sm:inline">返回展厅</span></Link><div className="text-center"><p className="text-sm font-black sm:text-base">童创艺境 · 管理中心</p><p className="hidden font-mono text-[9px] uppercase tracking-[.16em] text-slate-400 sm:block">Museum control room</p></div><Button variant="ghost" size="sm" onClick={() => { localStorage.removeItem('isAdmin'); setLoggedIn(false); }} className="rounded-full"><LogOut className="mr-2 size-4" />退出</Button></div></header>
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <section className="overflow-hidden rounded-[2rem] bg-slate-950 p-6 text-white sm:p-9"><div className="flex flex-col justify-between gap-7 sm:flex-row sm:items-end"><div><p className="font-mono text-[10px] font-black uppercase tracking-[.18em] text-white/45">Unified collection manager</p><h1 className="mt-3 text-3xl font-black tracking-[-.04em] sm:text-5xl">一处管理，三个展区</h1><p className="mt-3 max-w-xl text-sm leading-6 text-white/55">上传、编辑、删除和调整作品顺序。所有数据继续保存在原有 Supabase 中。</p></div><Button onClick={openAdd} className="h-12 rounded-full px-6 font-black" style={{ backgroundColor: definition.color, color: definition.foreground }}><Plus className="mr-2 size-5" />添加{zone === '3d' ? '3D' : '图片'}作品</Button></div></section>
...
function Login({ password, setPassword, error, login }: { password: string; setPassword: (value: string) => void; error: string; login: () => void }) { return <div className="museum-grid grid min-h-screen place-items-center bg-[#F7F7F4] p-4"><div className="w-full max-w-md rounded-[2rem] border border-slate-900/8 bg-white p-7 shadow-[0_30px_90px_rgba(15,23,42,.12)] sm:p-9"><span className="grid size-14 place-items-center rounded-2xl bg-[#E9EEFF] text-[#4E6BFF]"><Lock className="size-6" /></span><p className="mt-7 font-mono text-[10px] font-black uppercase tracking-[.16em] text-slate-400">Museum control room</p><h1 className="mt-2 text-3xl font-black">管理员登录</h1><p className="mt-2 text-sm text-slate-500">登录后可以管理三个展区的全部作品。</p><Input type="password" value={password} onChange={(event) => setPassword(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') login(); }} placeholder="输入管理员密码" className="mt-7 h-13 rounded-xl" />{error && <p className="mt-2 text-sm font-bold text-red-600">{error}</p>}<Button onClick={login} disabled={!password} className="mt-4 h-12 w-full rounded-full bg-slate-950 font-black text-white hover:bg-slate-800"><Lock className="mr-2 size-4" />进入管理中心</Button><Link href="/" className="mt-5 flex items-center justify-center gap-2 text-sm font-bold text-slate-500"><ArrowLeft className="size-4" />返回展厅</Link></div></div>; }
