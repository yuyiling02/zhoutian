...
export default function AdminPage() {
  ...
  return (
    <div className="museum-grid min-h-screen text-[#172033]" style={{ '--zone': definition.color, '--zone-soft': definition.softColor } as CSSProperties}>
      ...
      <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) { setEditing(null); setForm(emptyForm); } }}><DialogContent className="max-w-lg rounded-[1.75rem] border border-slate-900/10 bg-[#F5F2E9]"><DialogHeader><div className="mb-2 grid size-11 place-items-center rounded-2xl shadow-[4px_4px_0_rgba(23,32,51,.1)]" style={{ backgroundColor: definition.softColor, color: definition.color }}><ZoneIcon name={definition.icon} className="size-5" /></div><DialogTitle className="text-2xl font-black">{editing ? '编辑作品' : '添加作品'}</DialogTitle></DialogHeader><div className="space-y-4 py-3"><Field label="作品名称 *"><Input value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} className="h-12 rounded-xl bg-white" /></Field><Field label="作者姓名 *"><Input value={form.author} onChange={(event) => setForm({ ...form, author: event.target.value })} className="h-12 rounded-xl bg-white" /></Field>{zone === '3d' ? <><FileField id="model" label={`GLB 模型 ${editing ? '（不选则保留）' : '*'}`} accept=".glb,.gltf" value={form.model} onChange={(file) => setForm({ ...form, model: file })} /><FileField id="thumb" label="缩略图（可选）" accept="image/jpeg,image/png,image/webp" value={form.thumbnail} onChange={(file) => setForm({ ...form, thumbnail: file })} /></> : <FileField id="creative-image" label={`作品照片 ${editing ? '（不选则保留）' : '*'}`} accept="image/jpeg,image/png,image/webp" value={form.image} onChange={(file) => setForm({ ...form, image: file instanceof File ? file : null })} />}{progress !== null && <div><div className="mb-1 flex justify-between text-xs font-bold text-slate-500"><span>正在上传</span><span>{progress}%</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full transition-all" style={{ width: `${progress}%`, backgroundColor: definition.color }} /></div></div>}</div><DialogFooter><Button variant="outline" onClick={() => setDialogOpen(false)} className="soft-button rounded-full">取消</Button><Button onClick={() => void submit()} disabled={saving} className="rounded-full px-6 font-black shadow-[0_4px_0_rgba(23,32,51,.12)]" style={{ backgroundColor: definition.color, color: definition.foreground }}>{saving && <LoaderCircle className="mr-2 size-4 animate-spin" />}{saving ? '保存中…' : '保存作品'}</Button></DialogFooter></DialogContent></Dialog>
    </div>
  );
}
