...
import { ArrowDown, ArrowLeft, ArrowUp, Box, Check, Edit3, ImageIcon, ImagePlus, Layers3, LoaderCircle, Lock, LogOut, Plus, Search, Trash2, Upload, X } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const submit = async () => {
    if (!form.title.trim() || !form.author.trim()) { alert('请填写作品名称和作者姓名'); return; }
  ...
  return (
    <div className="museum-grid min-h-screen bg-[#F7F7F4] text-slate-950" style={{ '--zone': definition.color } as CSSProperties}>
      <header className="museum-glass sticky top-0 z-30 border-b border-[#4E6BFF]/10"><div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8"><Link href="/" className="inline-flex items-center gap-2 text-sm font-black"><ArrowLeft className="size-4" /><span className="hidden sm:inline">返回展厅</span></Link><div className="text-center"><p className="text-sm font-black sm:text-base">数创艺境 · 作品管理中心</p><p className="hidden text-[9px] font-bold tracking-[.13em] text-slate-400 sm:block">数智课堂创意作品平台</p></div><Button variant="ghost" size="sm" onClick={() => { localStorage.removeItem('isAdmin'); setLoggedIn(false); }} className="rounded-full"><LogOut className="mr-2 size-4" />退出</Button></div></header>
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <section className="tech-panel scan-accent overflow-hidden rounded-[2rem] p-6 sm:p-9"><span className="absolute right-7 top-6 font-mono text-[9px] font-bold tracking-[.16em] text-[#4E6BFF]/50">CONTROL · 001</span><div className="flex flex-col justify-between gap-7 sm:flex-row sm:items-end"><div><p className="font-mono text-[10px] font-black uppercase tracking-[.18em] text-[#4E6BFF]">Unified collection manager</p><h1 className="mt-3 text-3xl font-black tracking-[-.04em] sm:text-5xl">作品管理中心</h1><p className="mt-3 max-w-xl text-sm leading-6 text-slate-500">上传、编辑、删除和调整作品顺序，一个后台管理三个展区。</p></div><Button onClick={openAdd} className="h-12 rounded-full px-6 font-black shadow-[0_0_24px_rgba(78,107,255,.2)]" style={{ backgroundColor: definition.color, color: definition.foreground }}><Plus className="mr-2 size-5" />添加{zone === '3d' ? '3D' : '图片'}作品</Button></div></section>

        <section className="mt-5 grid gap-3 md:grid-cols-3">{zoneDefinitions.map((item) => { const active = item.id === zone; return <button key={item.id} type="button" onClick={() => switchZone(item.id)} className={`flex items-center gap-3 rounded-2xl border p-4 text-left transition ${active ? 'shadow-lg' : 'border-slate-900/8 bg-white hover:-translate-y-0.5'}`} style={active ? { backgroundColor: item.color, borderColor: item.color, color: item.foreground } : undefined}><span className="grid size-11 place-items-center rounded-xl" style={{ backgroundColor: active ? 'rgba(255,255,255,.18)' : item.softColor, color: active ? item.foreground : item.color }}><ZoneIcon name={item.icon} className="size-5" /></span><span className="min-w-0 flex-1"><span className="block truncate font-black">{item.title}</span><span className={`text-xs ${active ? 'opacity-70' : 'text-slate-500'}`}>{countFor(item.id)} 件作品</span></span></button>; })}</section>

        <section className="mt-5 grid gap-3 sm:grid-cols-3"><StatCard icon={<Layers3 className="size-5" />} label="作品总数" value={artworks.length + creative.length} color="#4E6BFF" /><StatCard icon={<Box className="size-5" />} label="3D 作品" value={artworks.length} color="#4E6BFF" /><StatCard icon={<ImageIcon className="size-5" />} label="图片作品" value={creative.length} color="#E84B4B" /></section>

        <section className="mt-5 rounded-[2rem] border border-slate-900/8 bg-white/85 p-4 shadow-sm sm:p-6">
      ...
      <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) { setEditing(null); setForm(emptyForm); } }}><DialogContent className="max-w-lg rounded-[1.75rem] border-0 bg-[#F7F7F4]"><DialogHeader><div className="mb-2 grid size-11 place-items-center rounded-2xl" style={{ backgroundColor: definition.softColor, color: definition.color }}><ZoneIcon name={definition.icon} className="size-5" /></div><DialogTitle className="text-2xl font-black">{editing ? '编辑作品' : '添加作品'}</DialogTitle></DialogHeader><div className="space-y-4 py-3"><Field label="作品名称 *"><Input value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} className="h-12 rounded-xl bg-white" /></Field><Field label="作者姓名 *"><Input value={form.author} onChange={(event) => setForm({ ...form, author: event.target.value })} className="h-12 rounded-xl bg-white" /></Field>{zone === '3d' ? <><FileField id="model" label={`GLB 模型 ${editing ? '（不选则保留）' : '*'}`} accept=".glb,.gltf" value={form.model} onChange={(file) => setForm({ ...form, model: file })} /><FileField id="thumb" label="缩略图（可选）" accept="image/jpeg,image/png,image/webp" value={form.thumbnail} onChange={(file) => setForm({ ...form, thumbnail: file })} /></> : <FileField id="creative-image" label={`作品照片 ${editing ? '（不选则保留）' : '*'}`} accept="image/jpeg,image/png,image/webp" value={form.image} onChange={(file) => setForm({ ...form, image: file instanceof File ? file : null })} />}{progress !== null && <div><div className="mb-1 flex justify-between text-xs font-bold text-slate-500"><span>正在上传</span><span>{progress}%</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full transition-all" style={{ width: `${progress}%`, backgroundColor: definition.color }} /></div></div>}</div><DialogFooter><Button variant="outline" onClick={() => setDialogOpen(false)} className="rounded-full">取消</Button><Button onClick={() => void submit()} disabled={saving} className="rounded-full px-6 font-black" style={{ backgroundColor: definition.color, color: definition.foreground }}>{saving && <LoaderCircle className="mr-2 size-4 animate-spin" />}{saving ? '保存中…' : '保存作品'}</Button></DialogFooter></DialogContent></Dialog>
    </div>
  );
}

function Login({ password, setPassword, error, login }: { password: string; setPassword: (value: string) => void; error: string; login: () => void }) { return <div className="museum-grid grid min-h-screen place-items-center bg-[#F7F7F4] p-4"><div className="tech-panel scan-accent w-full max-w-md rounded-[2rem] p-7 sm:p-9"><span className="grid size-14 place-items-center rounded-2xl bg-[#E9EEFF] text-[#4E6BFF] shadow-[0_0_28px_rgba(78,107,255,.18)]"><Lock className="size-6" /></span><p className="mt-7 font-mono text-[10px] font-black uppercase tracking-[.16em] text-[#4E6BFF]">Digital museum control</p><h1 className="mt-2 text-3xl font-black">数创艺境管理登录</h1><p className="mt-2 text-sm text-slate-500">登录后可以管理三个展区的全部作品。</p><Input type="password" value={password} onChange={(event) => setPassword(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') login(); }} placeholder="输入管理员密码" className="mt-7 h-13 rounded-xl bg-white/80" />{error && <p className="mt-2 text-sm font-bold text-red-600">{error}</p>}<Button onClick={login} disabled={!password} className="mt-4 h-12 w-full rounded-full bg-[#4E6BFF] font-black text-white shadow-[0_0_25px_rgba(78,107,255,.24)] hover:bg-[#405BE5]"><Lock className="mr-2 size-4" />进入管理中心</Button><Link href="/" className="mt-5 flex items-center justify-center gap-2 text-sm font-bold text-slate-500"><ArrowLeft className="size-4" />返回展厅</Link></div></div>; }

function StatCard({ icon, label, value, color }: { icon: React.ReactNode; label: string; value: number; color: string }) { return <article className="tech-panel rounded-2xl p-5"><div className="flex items-center justify-between"><span className="grid size-10 place-items-center rounded-xl" style={{ backgroundColor: `${color}16`, color }}>{icon}</span><span className="font-mono text-[9px] tracking-[.16em] text-slate-400">LIVE DATA</span></div><p className="mt-5 text-sm font-bold text-slate-500">{label}</p><p className="mt-1 text-3xl font-black tracking-[-.04em]">{value}<span className="ml-1 text-xs font-bold text-slate-400">件</span></p></article>; }

function ArtworkList({ items, allItems, query, move, edit, remove }: { items: Artwork[]; allItems: Artwork[]; query: string; move: (index: number, direction: -1 | 1) => void; edit: (work: Artwork) => void; remove: (work: Artwork) => void }) { if (!items.length) return <Empty />; return <div className="mt-6 space-y-3">{items.map((work) => { const index = allItems.findIndex((item) => item.id === work.id); return <article key={work.id} className="grid gap-4 rounded-2xl border border-slate-900/8 bg-white p-4 sm:grid-cols-[72px_1fr_auto] sm:items-center"><div className="relative size-18 overflow-hidden rounded-xl bg-[#E9EEFF]">{work.thumbnail ? <Image src={work.thumbnail} alt="" fill sizes="72px" className="object-cover" /> : <span className="grid size-full place-items-center text-[#4E6BFF]"><Box className="size-6" /></span>}</div><div className="min-w-0"><h3 className="truncate font-black">{work.title}</h3><p className="mt-1 truncate text-sm text-slate-500">{work.author} · ID {work.id}</p></div><RowActions index={index} length={allItems.length} query={query} move={move} edit={() => edit(work)} remove={() => remove(work)} /></article>; })}</div>; }
function CreativeList({ items, allItems, query, zone, move, edit, remove }: { items: CreativeWork[]; allItems: CreativeWork[]; query: string; zone: CreativeCategory; move: (index: number, direction: -1 | 1) => void; edit: (work: CreativeWork) => void; remove: (work: CreativeWork) => void }) { const def = getZoneDefinition(zone); if (!items.length) return <Empty />; return <div className="mt-6 grid gap-4 md:grid-cols-2">{items.map((work) => { const index = allItems.findIndex((item) => item.id === work.id); return <article key={work.id} className="overflow-hidden rounded-2xl border border-slate-900/8 bg-white"><div className="relative aspect-[16/9]" style={{ backgroundColor: def.softColor }}><Image src={work.imageUrl} alt={work.title} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover" /></div><div className="p-4"><h3 className="truncate font-black">{work.title}</h3><p className="mt-1 truncate text-sm text-slate-500">{work.author}</p><div className="mt-4"><RowActions index={index} length={allItems.length} query={query} move={move} edit={() => edit(work)} remove={() => remove(work)} /></div></div></article>; })}</div>; }
