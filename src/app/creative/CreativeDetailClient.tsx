...
export default function CreativeDetailClient() {
  ...
  const share = async () => {
    ...
    if (navigator.share) { try { await navigator.share({ title: `${work.author} · ${work.title || '无题作品'}`, text: `看看数创艺境的${zone.title}作品`, url }); return; } catch { return; } }
  ...
  return (
    <div className="museum-grid min-h-screen bg-[#F7F7F4] text-slate-950">
      ...
      <main className="mx-auto max-w-7xl px-4 py-7 sm:px-6 sm:py-12 lg:px-8">
        ...
        <section className="tech-panel scan-accent relative overflow-hidden rounded-[2rem] p-2 sm:p-4"><div className="relative min-h-[60vh] w-full overflow-hidden rounded-[1.5rem]" style={{ backgroundColor: zone.softColor }}><Image src={work.imageUrl} alt={work.title || `${work.author}的作品`} fill priority sizes="100vw" className="object-contain" /></div></section>
        <section className="mt-7 grid gap-4 sm:grid-cols-2"><div className="tech-panel rounded-[1.5rem] p-6"><p className="font-mono text-[10px] font-black uppercase tracking-[.16em]" style={{ color: zone.color }}>Artwork</p><h2 className="mt-3 text-2xl font-black">{work.title || '无题作品'}</h2><p className="mt-2 text-sm text-slate-500">{zone.title} · 图片作品</p></div><div className="tech-panel rounded-[1.5rem] p-6"><p className="font-mono text-[10px] font-black uppercase tracking-[.16em]" style={{ color: zone.color }}>Creator</p><h2 className="mt-3 text-2xl font-black">{work.author}</h2><p className="mt-2 text-sm text-slate-500">数创艺境参展创作者</p></div></section>
      </main>
    </div>
  );
}
