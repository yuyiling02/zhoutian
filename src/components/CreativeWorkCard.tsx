  ...
  return (
    <Link href={`/creative?id=${encodeURIComponent(work.id)}`} className="group block overflow-hidden rounded-[1.75rem] border border-slate-900/12 bg-white outline-none transition duration-300 hover:-translate-y-1.5 hover:border-[var(--zone)] focus-visible:ring-4" style={{ '--tw-ring-color': `${zone.color}35`, '--zone': zone.color, boxShadow: `0 10px 0 ${zone.color}14` } as CSSProperties}>
      <div className="relative aspect-[4/5] overflow-hidden bg-slate-100">
        <Image src={work.imageUrl} alt={work.title || (work.author ? `${work.author}的作品` : '未命名作品')} fill priority={priority} sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-cover transition duration-500 group-hover:scale-[1.035]" />
      ...
      <div className="flex items-center justify-between gap-3 px-5 py-4">
        <div className="min-w-0"><h3 className="truncate text-lg font-black tracking-tight text-slate-950">{work.title || '无题作品'}</h3><p className="mt-1 truncate text-sm text-slate-500">创作者 · {work.author || '未署名'}</p></div>
