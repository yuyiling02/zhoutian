...
export function ArtworkCard({ artwork, priority = false }: ArtworkCardProps) {
  ...
  return (
    <article className="group w-full overflow-hidden rounded-[1.75rem] border border-slate-900/12 bg-white shadow-[0_10px_0_rgba(60,92,255,.08)] transition duration-300 hover:-translate-y-1.5 hover:border-[#3C5CFF]/35 hover:shadow-[0_14px_0_rgba(60,92,255,.13)]">
      <div className="relative aspect-[4/5] overflow-hidden bg-[#E9EDFF]">
        {artwork.thumbnail && !imageError ? (
          ...
          <div className="grid size-full place-items-center bg-[#E9EDFF] text-center text-[#3C5CFF]">
        ...
        <span className="absolute left-4 top-4 inline-flex items-center gap-1.5 rounded-full border border-white/70 bg-white/90 px-3 py-1.5 text-xs font-black text-slate-900 shadow-sm backdrop-blur"><ZoneIcon name="paint-space" className="size-3.5 text-[#3C5CFF]" />绘画立体</span>
      ...
      <div className="flex items-center justify-between gap-3 px-5 py-4">
        ...
        <span className="grid size-10 shrink-0 place-items-center rounded-2xl bg-[#E9EDFF] text-[#3C5CFF]"><ZoneIcon name="paint-space" className="size-5" /></span>
      </div>
    </article>
  );
}
